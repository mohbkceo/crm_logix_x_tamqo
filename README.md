...
| `DELIVERY_LATEST_PATH`, `DELIVERY_LATEST_METHOD` | Unverified updates route; disabled while path is blank |
| `DELIVERY_RESPONSE_PACKAGES_PATH` | Configurable response array path, default convention `Package` |
| `DELIVERY_RESPONSE_TRACKING_FIELD` | Configurable tracking field, default convention `Tracking` |
| `DELIVERY_RESPONSE_STATUS_FIELD` | Configurable status field, default convention `Status` |
...
- `GET /token` — credential endpoint connectivity test.
- `POST /add_colis` — `{ Package: [...] }` shipment creation.
- `POST /lire` — `{ Package: [{ Tracking }] }` parcel lookup.
...
The documented request mapping includes Tracking, DeliveryType (`0` home / `1` desk), TypeColis (`0` normal / `1` exchange), Confirmed, Client, MobileA/B, Address, IDWilaya, Commune, Total, Note, Product, immutable order number as id_Externe, and source snapshot as Source. The creation workflow uses an empty Confirmed field; readiness has its own action.

### Documentation ambiguity and response verification

The document labels **GET `/tarification`** as “Retrieving the latest updated packages”, while **POST `/tarification`** is pricing. `getLatestUpdatedPackages()` remains disabled unless the method/path are explicitly configured. No alternate URL was invented.

The document does **not** define success/error response schemas or provider status codes. `Package`, `Tracking`, and `Status` are configurable parsing conventions, not a claimed verified provider response contract. Confirm them with ABEX using actual responses and adjust the response paths. Leave `DELIVERY_STATUS_MAP={}` until verified codes are available. Unknown statuses are retained without changing the order. The credential test reports endpoint connectivity only, because a 200 response cannot prove provider-level authentication success without its contract.

Known mapped forward status jumps can skip unobserved stages; only the actual observed transition is recorded. Invalid regressions and terminal-state changes are retained as sync errors for review. Ready requests are followed by a tracking refresh and require a mapped readiness status before being called verified.
